...
**UI options:** Scalar (`app.MapScalarApiReference()`, what the docs lead with) or keep Swagger UI without SwaggerGen (`Swashbuckle.AspNetCore.SwaggerUi` + `app.UseSwaggerUI(o => o.SwaggerEndpoint("/openapi/v1.json", "v1"))`). Gate any UI behind `IsDevelopment()`.

**Update `launchUrl` in every `launchSettings.json` profile for the project — don't skip this, it's easy to forget and the failure is silent.** `dotnet run`/F5 auto-opens the browser to whatever `launchUrl` says; a value left over from Swashbuckle (`"launchUrl": "swagger"`) now points at a route that no longer exists (`Swashbuckle` is gone, so `/swagger` 404s) — nothing in the build or startup logs flags this, it just quietly opens a 404 page, and it's easy to mistake for "the OpenAPI migration didn't work" when it's actually just a stale launch profile. `Scalar.AspNetCore`'s default endpoint is `/scalar` (redirects to `/scalar/v1`, or whatever document name is configured), so `launchUrl` should be `"scalar"`, not `"swagger"` or `"openapi"`. Check **every** profile in the file (commonly two: the project's own profile and `IIS Express`) — grep `launchSettings.json` for `"launchUrl"` across the solution rather than fixing just the one profile you happen to run.

**If the project builds controller routes through a custom `IApplicationModelConvention` instead of literal `[Route]`/`[HttpGet]` attributes, `AddOpenApi()`'s document can come back with zero paths even though routing itself works fine.** ASP.NET Core decides `ApiExplorerModel.IsVisible` during `DefaultApplicationModelProvider`'s `OnProvidersExecuting` pass, against the *reflection-discovered* model — before any convention registered via `MvcOptions.Conventions` has run (those apply later, in `OnProvidersExecuted`). If no action has a literal routing attribute in source, every action still looks conventionally-routed at the point that visibility decision is made, so `ApiExplorerModel.IsVisible` never becomes `true` for any of them — regardless of the fact that the convention correctly gives every action a working attribute route immediately afterward. This silently affects *every* OpenAPI/Swagger generator that reads from `IApiDescriptionGroupCollectionProvider` — old Swashbuckle included, so it's not something this upgrade introduces, but an OpenAPI/Scalar adoption pass is often the first thing to actually notice it (nobody was checking Swagger's endpoint list before). Symptom: the document generator itself works fine — a throwaway Minimal API endpoint added alongside the controllers shows up in the document immediately — but the document has `"paths": {}` regardless of how many controllers exist.

Diagnose without a decompiler by comparing two counts from a throwaway diagnostic endpoint: `IActionDescriptorCollectionProvider.ActionDescriptors.Items.OfType<ControllerActionDescriptor>().Count()` (does routing work?) against `IApiDescriptionGroupCollectionProvider.ApiDescriptionGroups.Items.SelectMany(g => g.Items).Count()` (does ApiExplorer know about it?). A big gap — e.g. 85 routed actions vs. 0 ApiExplorer entries — confirms this exact bug.

**`[ApiController]` is not a fix here and can make things worse.** It enforces attribute routing during that *same* early `OnProvidersExecuting` phase (via `ApiBehaviorApplicationModelProvider`), so adding it to a shared controller base class in this situation throws `InvalidOperationException: ... does not have an attribute route. Action methods on controllers annotated with ApiControllerAttribute must be attribute routed.` at startup — a hard crash affecting every action, not a silent gap. The actual fix: wherever the custom convention synthesizes the route (typically a line like `selectorModel.AttributeRouteModel = new AttributeRouteModel(...)`), also set `action.ApiExplorer.IsVisible = true` right there — that convention is already the place establishing that the action is properly routed, so it's the correct place to tell ApiExplorer that too.
...
- **Adding `Microsoft.Azure.Functions.Worker.OpenTelemetry` can raise the solution's `Microsoft.Azure.Functions.Worker.Core` floor past what the pinned `Worker`/`Worker.Grpc` versions support** — under CPM this restores and builds cleanly but fails every function invocation at runtime with a `MissingMethodException` (e.g. on `DefaultTraceContext`). Not specific to OpenTelemetry — any Functions extension package can do this — but it's the one most upgrades add in this exact phase. See `06-functions.md`'s Worker/Worker.Grpc/Worker.Core lockstep note before adding it.
